...
// Factory 본문에 나타난 source 저장 프로퍼티 참조 위치 집합
package struct GraphSourceReferences {
	// bare source 저장 프로퍼티 참조 위치와 이름
	package let bare: [Int: String]
	// `self.source` 저장 프로퍼티 참조 위치와 이름
	package let explicitSelf: [Int: String]
	// bare closure capture source 저장 프로퍼티 참조 위치와 이름
	package let bareCapture: [Int: String]

	// Factory가 실제로 읽는 source 저장 프로퍼티 이름 집합
	package var sourceNames: Set<String> {
		Set(bare.values)
...
// lexical scope를 반영해 Factory의 source 저장 프로퍼티 참조 수집
package func graphSourceReferences(
	in factory: FunctionDeclSyntax,
	sourceNames: Set<String>
) -> GraphSourceReferences {
	guard let body = factory.body else {
		return GraphSourceReferences(bare: [:], explicitSelf: [:], bareCapture: [:])
	}
...
private final class SourceGraphReferenceFinder: SyntaxVisitor {
	...
	var references: GraphSourceReferences {
		GraphSourceReferences(bare: bare, explicitSelf: explicitSelf, bareCapture: bareCapture)
	}
	...
	private func recordSourceGraphReference(
		...
		switch kind {
		case .bare:
		bare[graphSourceOffset(of: token)] = name
		case .explicitSelf:
		explicitSelf[graphSourceOffset(of: token)] = name
		case .bareCapture:
		bareCapture[graphSourceOffset(of: token)] = name
		}
	}
}

// SwiftSyntax token의 source file UTF-8 위치
package func graphSourceOffset(of token: TokenSyntax) -> Int {
