...
func typedOverrideDeclarations(
	...
	return overrides.map(selectionDeclaration)
		+ [builderDeclaration(named: builderName, graph: graph.name, providers: overrides, accessLevel: accessLevel)]
...
private func builderDeclaration(
	...
	return DeclSyntax(
		...
		\(raw: accessLevel.rawValue) struct \(builderName) {
		    ...
		    \(raw: accessLevel.rawValue) func build() -> \(graph) {
		        \(graph)(overrides: self)
		    }
		}
		"""
	)
...
private func graphInitializers(
	...
	let storageAssignment = storage.initializationAssignment()
	let originalInit = DeclSyntax(
		"""
		\(raw: accessLevel.rawValue) convenience init() {
		    self.init(overrides: Self.override())
		}
		"""
	)
	let overrideInit = DeclSyntax(
		"""
		private init(overrides: \(builderName)) {
...
private func typedOverridePropertyDeclarations(
	...
	return transientStorage + storage.declarations() + properties
}
...
private struct TypedOverrideSharedStorage {
	...
	func declarations() -> [DeclSyntax] {
		...
		private static func \(builderName)(_ overrides: \(overrideBuilderName)) -> \(typeName) {
		    \(raw: constructions)
	...
	func initializationAssignment() -> String {
		...
		return "self.\(propertyName) = Self.\(builderName)(overrides)"
	}
	...
	private func helperDeclaration(for override: TypedOverrideProvider) -> DeclSyntax? {
		guard let name = helperNames[override.provider.registrationIdentity],
			let body = sharedHelperBody(for: override.provider.factory) else {
			return nil
		}
		...
		return DeclSyntax("""
		private static func \(name)(\(raw: parameters)) -> \(raw: override.provider.returnType.trimmedDescription) \(raw: body)
		""")
	}
