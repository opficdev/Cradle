...
///
/// 제네릭 매개변수와 `where` 절이 없는 `final class` 또는 `actor`에 적용합니다. 일반 `@Provide` Factory의 반환 타입을 기준으로 graph에 읽기 전용 생성 프로퍼티를 추가합니다.
///
/// 자세한 사용 조건과 제약은 <doc:DependencyGraph>에서 설명합니다.
///
/// - Parameters:
///   - lifetime: graph 인스턴스의 보유 범위입니다. 기본 `.instance`는 호출자가 만든 graph를 사용하고, `.shared`는 프로세스 동안 보유하는 `static let shared` graph를 만듭니다.
///   - sources: `final class` graph가 보관하고 Factory에서 읽을 source graph 타입 배열입니다.
...
///
/// `@DependencyGraph` 본체에 직접 선언한 동기 `private` 인스턴스 메서드에 적용합니다. Factory 결과는 graph를 만들 때 한 번 생성되고 해당 graph가 보관합니다.
///
/// 수명 정책과 외부 입력 사용법은 <doc:DependencyGraph>에서 설명합니다.
...
///
/// `@DependencyGraph` 본체에 직접 선언한 동기 `private` 인스턴스 메서드에 적용합니다. 외부 입력이 없는 Factory의 반환 타입이 등록 타입과 생성 접근자의 타입이 됩니다. `@External` 입력이 있는 `.transient` Factory는 호출 시점 생성 메서드를 만듭니다.
///
/// - Parameter lifetime: Factory 결과의 graph별 보유 정책입니다. `.shared`, `.lazy`, `.transient`의 차이는 <doc:DependencyGraph>에서 설명합니다.
@attached(peer, names: arbitrary)
@attached(body)
